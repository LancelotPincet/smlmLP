...
class points(DataFrame) :
    ...
    @column(headers=['point'], dtype=np.uint64, fill=0, save=True, agg='min', index="detections")
    def pnt(self) :
    ...
    @column(headers=['x stable [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def xx(self) :
        """Return drift-corrected x coordinates."""
        if not self.dx_exists : return "x"
        return self.x - self.dx

    @column(headers=['y stable [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def yy(self) :
        """Return drift-corrected y coordinates."""
        if not self.dy_exists : return "y"
        return self.y - self.dy

    @column(headers=['z stable [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def zz(self) :
    ...
    @column(headers=['x [nm]', 'xnm'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def x(self) :
    ...
    @column(headers=['y [nm]', 'ynm'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def y(self) :
    ...
    @column(headers=['x modloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def x_modloc(self) :
    ...
    @column(headers=['y modloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def y_modloc(self) :
    ...
    @column(headers=['x timeloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def x_timeloc(self) :
    ...
    @column(headers=['y timeloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def y_timeloc(self) :
    ...
    @column(headers=['z [nm]', 'znm'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def z(self) :
    ...
    @column(headers=['z modloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_modloc(self) :
    ...
    @column(headers=['z timeloc [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_timeloc(self) :
    ...
    @column(headers=['z astigmatism [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_astig(self) :
    ...
    @column(headers=['z biplane [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_biplane(self) :
    ...
    @column(headers=['z donald [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_donald(self) :
    ...
    @column(headers=['z miet [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_miet(self) :
    ...
    @column(headers=['z qtirf [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_qtirf(self) :
    ...
    @column(headers=['azimuth [deg]'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def azimuth(self) :
    ...
    @column(headers=['azimuth polar2d [deg]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['azimuth polar3d [deg]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['tilt [deg]'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def tilt(self) :
    ...
    @column(headers=['tilt polar3d [deg]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['phase [rad]'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def phase(self) :
    ...
    @column(headers=['phase demodulated [rad]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['phase sequential [rad]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['lifetime [ns]'], dtype=np.float32, fill=np.nan, save=False, agg='mean')
    def lifetime(self) :
    ...
    @column(headers=['lifetime tcspc [ns]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def lifetime_tcspc(self) :
        """Return explicit TCSPC lifetime values."""
        return None

    @column(headers=['lifetime iflim [ns]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['lifetime dpflim [ns]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    ...
    @column(headers=['x frequency [hz]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def x_freq(self) :
        """Return explicit x frequency values."""
        return None

    @column(headers=['y frequency [hz]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def y_freq(self) :
        """Return explicit y frequency values."""
        return None

    @column(headers=['z frequency [hz]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def z_freq(self) :
    ...
    @column(headers=['spectral ratio'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def spectral_ratio(self) :
        """Compute spectral y/x ratio."""
        return self.spectral_y / self.spectral_x

    @column(headers=['spectral x intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def spectral_x(self) :
    ...
    @column(headers=['spectral y intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def spectral_y(self) :
    ...
    @column(headers=['biplane ratio'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def biplane_ratio(self) :
        """Compute biplane y/x ratio."""
        return self.biplane_y / self.biplane_x

    @column(headers=['biplane x width [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def biplane_x(self) :
    ...
    @column(headers=['biplane y width [nm]'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def biplane_y(self) :
    ...
    @column(headers=['donald ratio'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def donald_ratio(self) :
        """Compute DONALD y/x ratio."""
        return self.donald_y / self.donald_x

    @column(headers=['donald x intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def donald_x(self) :
    ...
    @column(headers=['donald y intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def donald_y(self) :
    ...
    @column(headers=['iflim ratio'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def iflim_ratio(self) :
        """Compute IFLIM y/x ratio."""
        return self.iflim_y / self.iflim_x

    @column(headers=['iflim x intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def iflim_x(self) :
    ...
    @column(headers=['iflim y intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def iflim_y(self) :
    ...
    @column(headers=['dpflim ratio'], dtype=np.float32, fill=np.nan, save=True, agg='mean')
    def dpflim_ratio(self) :
        """Compute DPFLIM y/x ratio."""
        return self.dpflim_y / self.dpflim_x

    @column(headers=['dpflim x intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def dpflim_x(self) :
    ...
    @column(headers=['dpflim y intensity [photon]'], dtype=np.float32, fill=0, save=True, agg='mean')
    def dpflim_y(self) :
