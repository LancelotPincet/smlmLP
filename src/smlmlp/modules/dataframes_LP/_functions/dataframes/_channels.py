...
# %% Libraries
from numpy._core import False_
from smlmlp import DataFrame, column, index_channels
import numpy as np
...
class channels(DataFrame) :
    ...
    @column(headers=['psf x sigma [nm]'], save=True, index=False, agg='mean')
    ...
    @column(headers=['psf y sigma [nm]'], save=True, index=False, agg='mean')
    ...
    @column(headers=['psf theta [deg]'], save=True, index=False, agg='mean')
    ...
    @column(headers=['registration x shear'], save=True, index=False, agg='mean')
    def x_shear(self:np.float32) :
        return np.asarray([self.locs.config.channels_x_shear[i-1] for i in self.ch], dtype=np.float32)

    @column(headers=['registration y shear'], save=True, index=False, agg='mean')
    ...
    @column(headers=['x cropshape [pix]'], save=True, index=False, agg='mean')
    ...
    @column(headers=['y cropshape [pix]'], save=True, index=False, agg='mean')
    def y_cropshape(self:np.float32) :
        return np.asarray([self.locs.config.channels_crops_pix[i-1][1] for i in self.ch], dtype=np.float32)
    
    @column(headers=['x cropsize [nm]'], save=False, index=False, agg='mean')
    def x_cropsize(self:np.float32) :
        return self.x_cropshape * self.x_pixel

    @column(headers=['y cropsize [nm]'], save=False, index=False, agg='mean')
