...
class Config() :
            ...
            if isinstance(config, str) or isinstance(config,Path('').__class__):
                config_file = Path(config).with_suffix('.json')
                    if config_file.exists() :
                        ...
                        raise SyntaxError(f'config string was not recognized: {config}')
    ...
    def xpixel_cam(self) :
        return 6.5
    @metadatum("Pixel size", unit='nm', ui=False)
    def xpixel(self) : # pixel size in x [nm]
        return self.xpixel_cam * 1e3 / self.det_mag / self.obj_mag
    @xpixel.setter
    def xpixel(self, value) :
        self.xpixel_cam = value * 1e-3 * self.det_mag * self.obj_mag
    @metadatum("Pixel size", unit='µm')
    def ypixel_cam(self) :
        return 6.5
    @metadatum("Pixel size", unit='nm', ui=False)
    def ypixel(self) : # pixel size in x [nm]
        return self.ypixel_cam * 1e3 / self.det_mag / self.obj_mag
    @ypixel.setter
    def ypixel(self, value) :
        self.ypixel_cam = value * 1e-3 * self.det_mag * self.obj_mag
    @metadatum("Pixel size")
    def obj_mag(self) :
        return 100 # Objective (+TL) magnitudes
    @metadatum("Pixel size")
    def det_mag(self) :
        return 2 / 3 # Other detection magnitudes
    @property
    def pixel(self) : # pixel size [nm]
        return np.sqrt(self.xpixel * self.ypixel)
    @pixel.setter
    def pixel(self, value) :
        self.xpixel, self.ypixel = value, value
