...
# %% Function
@block()
def bkgd_spatial_mean(channels, /, channels_mean_radius_pix=7., bkgds=None, noise_corrections=None, *, cuda=False, parallel=False) :
    '''
    This function creates the spatial local mean background.
    '''

    # Get channels_mean_radius_pix
    try :
        if len(channels_mean_radius_pix) != len(channels) :
            if len(channels_mean_radius_pix) == 2 :
                channels_mean_radius_pix = [channels_mean_radius_pix for _ in range(len(channels))]
            else :
                raise ValueError('channels_mean_radius_pix does not have the same length as channels')
    except TypeError:
        channels_mean_radius_pix = [(channels_mean_radius_pix, channels_mean_radius_pix) for _ in range(len(channels))]
    ...
    for i in range(len(channels)) :
        ...
        channel = channels[i]
        sigma = channels_mean_radius_pix[i][0] / 2, channels_mean_radius_pix[i][1] / 2
