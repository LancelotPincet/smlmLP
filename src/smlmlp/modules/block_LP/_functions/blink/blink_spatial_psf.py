...
# %% Function
@block()
def blink_spatial_psf(channels, /, crop_pix=41, *, channels_pixels_nm=100., cuda=False, parallel=False) :
    '''
    ...
    for channel, pix in zip(channels, channels_pixels_nm) :
        ...
        bounds = ([0., 0., 0.75, -0.25, 0.], [cp, cp, 1.25, 0.25, 90.]) # (lower, upper)
        ...
        mask[cp, :] = False
        mask[:, cp] = False
        _psf = psf[mask].reshape(2*cp, 2*cp)
        _yy = yy[mask].reshape(2*cp, 2*cp)
        _xx = xx[mask].reshape(2*cp, 2*cp)
